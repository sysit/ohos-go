...
> **两份资产于 2026-09-24 在各自宿主上重新打包**（darwin 在本机、linux 在 238 那台）。
> 与 beta2 那两份相比，**工具链里变了的只有宿主侧的 `-exec` 包装**（修已知问题 7d）——
> `bin/go`、`bin/gofmt`、编译器、stdlib **逐字节相同**（`bin/go` = `9cfddd979ec1c64f…`）。
> 所以从 beta2 升上来**不需要动任何构建脚本**，`go version` 也逐字相同。
>
> **树指纹**（定义见 `misc/openharmony/runtests.sh`：`bin/` 顶层文件的 sha256 汇总，**不含子目录**）
> 分别是 darwin `bc958b2dca13…`、linux `3e8b4ece4304…`。
...
- **溯源**：载体量的工具链指纹 `743723f32866…` = 它当时用的 `~/go1.27.1-ohos`（darwin）
  ...
  **别把这个指纹拿去对新资产**：那个口径算的是**整棵 `bin/`**，里面含 `-exec` 包装，
  而 7c/7d 两次修的正是包装 —— 现在的 darwin 资产指纹是 `bc958b2dca13…`。
...
**本次两份资产是另一棵树**（又修了 7d），那一轮的指纹是 `bc958b2dca13…`。

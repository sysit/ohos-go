...
| 件 | 位置 | 作用 |
|---|---|---|
| `-exec` 包装 | `misc/go_openharmony_exec` | 被 `go test` / `go run` 经 `go_<GOOS>_<GOARCH>_exec` 约定自动发现，push 到设备执行并回传真实退出码。**并把包目录镜像到设备、从镜像里运行**（§7 阶段 0.4） |
...
- **【硬前提】`CC` 必须写 SDK clang 的绝对路径。** 裸 `clang` 会解析到 `/usr/bin/clang`（Apple clang），它不认 `aarch64-linux-ohos`，报 `posix_spawn failed: No such file or directory`。注意 `CLAUDE.md` 与 skill §6 的示例写的都是裸 `clang`，只在 SDK clang 恰好排 PATH 最前时才成立 —— **这是文档缺口，照抄会中招**：
  ```bash
...
| 包 | 症状 | 性质 |
|---|---|---|
| `time` | `panic: cannot load America/Los_Angeles …` | **可修**。`initTestingZone()` 为求 hermetic **只用** `../../lib/time/zoneinfo.zip`（相对包目录）。镜像只推了包目录本身，`../../lib` 在设备上不存在。android 版靠 `adbCopyTree` 向上走解决 —— 即 `pushSourceTree` 那条 `ponytail:` 天花板 |
| `runtime` | `panic: open /Users/xiphis/projects/ohos-go/src/runtime/traceback_system_test.go: no such file or directory` | **不可修（除非 `-trimpath`）**。`formatStack` 拿到的是**编译期绝对路径**，直接当绝对路径 `open`。设备上要存在 `/Users/xiphis/…` 只能靠 root 建符号链接。**这一条要登记为已知不可跑，不要试图修包装** |
...
|---|---|
| PASS | **224** |
| FAIL | **38**（去重后；`results.tsv` 跨轮追加，同行数 45 是重复行） |
| TIMEOUT | **0** |
| WRAPPER | **0** |

TIMEOUT/WRAPPER 双零 = harness 侧没有噪音，**38 个 FAIL 全是真信号**。逐条 triage 后：

**37 条落进已知类，1 条是真 port 缺陷（`crypto/internal/fips140only`，已修，见下）。** 已知类的分布：

| 类 | 包数 | 代表包 | 决定性证据 |
|---|---|---|---|
| 环回 TCP/UDP 被禁 | 19 | `net`、`net/http`、`net/rpc`、`net/smtp`、`log/syslog`、`encoding/json`、`context`、`net/http/{cgi,httptest,httputil,cookiejar,pprof,internal/http2}`、`compress/gzip`、`crypto/tls` | `bind: permission denied`（`crypto/tls` 与 `net/http/pprof` 是 `init()` 里就 listen，连一个测试都没跑到，日志只有 3 行 / 13 行） |
...
| 设备上没有 `go` / 宿主源码树 | 8 | `go/build`、`go/types`、`go/parser`、`internal/copyright`、`go/internal/gccgoimporter`、`math/big/internal/asmgen`、`path/filepath`、`runtime` | `lstat /Users/xiphis/projects/ohos-go/src/unicode: no such file or directory`、`'go build' unavailable: exec: "go": executable file not found in $PATH`、`../../arith_386.s`。**与 §5.1 的 `runtime` 同类，不可修**（除非 `-trimpath` 或把 GOROOT 整棵推到设备） |
| 镜像只覆盖包目录本身 | 3 + 若干 | `time`、`io/ioutil`、`compress/{flate,lzw,zlib}`、`internal/zstd`、`image/{draw,gif,jpeg}` | 路径是 `../testdata/`、`../../testdata/`、`../../lib/time/zoneinfo.zip`、`..`。**方向是「向上或向旁」**，`pushSourceTree` 只推包目录自身。与 §5.1 的 `time` 同一根因，**可修** |

`internal/copyright` 值得单记一笔：它 `filepath.WalkDir(GOROOT/src)`，walk 在设备上必然报错，而回调里**没判 `err` 就对 `d` 取 `d.IsDir()`** → nil 解引用 panic。这是上游测试的健壮性问题，但触发条件仍是「GOROOT 不在设备上」，归上一类。

**唯一一条真 port 缺陷：`crypto/internal/fips140only`。** 详见 §5.4。
...
| 风险 | 说明 |
|---|---|
| ~~harness 缺陷淹没真信号~~（已按 (a) 处置） | 原风险：包装缺「建工作目录 + 推源文件树 + 下发 env」，会让 cwd/testdata/env 三类测试成片 FAIL。**2026-09-21 选了 (a)：cwd 与 testdata 两类已在包装里补掉并实测兑现**（§7 阶段 0.4）。**残留**：env 透传仍未做，`crypto/x509` 一类仍不可测；此外「环回 TCP listen 被禁」「设备权限模型」是**真·环境限制，不是 harness 缺口也不是 port 缺陷**，三类必须在 FAIL 标注里分开 |
...
| **下游拿到过期工具链** | `~/go1.27.1-ohos` 就是 v2rayHM 的 `OHOS_GO_ROOT`。它现在缺 PIE 修复 → 下游编出的 cgo 产物会 `Signal 11`。**这是本轮顺带发现的、超出本方案范围的真问题**，按下游政策只报告不改。 |

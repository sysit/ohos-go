	...
	case "asmcheck", "objcheck":
		...
		for _, env := range ops.Envs() {
			...
			if action == "objcheck" {
				...
				dumpcmd := exec.Command(goTool, dumpline...)
				dumpcmd.Env = append(os.Environ(), env.Environ()...)
				dumpcmd.Stdout, dumpcmd.Stderr = &buf, &buf
				if err := dumpcmd.Run(); err != nil {
					lastErr = err
					t.Log(env, "\n", dumpcmd.Stderr)
				} else if err := t.objCheck(buf.String(), t.gorootTestDir, t.goFileName(), env, ops[env]); err != nil {
					lastErr = err
					t.Log(err)
				}
			} else if err := t.asmCheck(buf.String(), long, env, ops[env]); err != nil {
				lastErr = err
				t.Log(err)
			}
			if action == "objcheck" {
